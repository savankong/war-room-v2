/**
 * POST /.netlify/functions/seed-orgs
 * Clears existing org/people data and loads the Department of War org chart CSV.
 * Protected by a simple bearer token: Authorization: Bearer <SEED_SECRET>
 */
import type { Context } from '@netlify/functions';
import { getDatabase } from '@netlify/database';

// ── Inline CSV data ────────────────────────────────────────────────────────
// (embedded so no filesystem access is needed in the serverless function)
const CSV = `Level,Organization,Parent_Organization,Leader_Name,Leader_Title,Service_Branch,Location,Description
...
3,Office of Science,Department of Energy,Dr. Darío Gil,Under Secretary for Science,Federal,Washington DC,DOE science
...
function badgeFromBranch(branch: string): Badge | null {
...
function avatarColor(branch: string): string {
  const map: Record<string, string> = {
    'Army': '#4a7c29', 'DoN': '#1a3a6b',
    'Air Force': '#1e5fa5', 'Air Force/Space Force': '#1e5fa5',
    'Space Force': '#1c2951', 'Coast Guard': '#c8502d',
    'DoW': '#283a6b', 'DoD': '#283a6b', 'All': '#283a6b', 'Federal': '#6b5228',
  };
  return map[branch] ?? '#283a6b';
}

interface Row { level: number; org: string; parent: string; leaderName: string; leaderTitle: string; branch: string; location: string; description: string; }
...
export default async function handler(req: Request, _ctx: Context) {
  ...
  try {
    ...
    return Response.json({ ok: false, error: (err as Error).message, log }, { status: 500 });
  }
}
