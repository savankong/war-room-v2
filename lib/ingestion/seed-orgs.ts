...
async function seed() {
  ...
  for (const [name, row] of seen) {
    ...
    const [r] = await db`
      ...
      VALUES (${name}, ${slug}, ${b?.text ?? null}, ${b?.color ?? null},
              ${row[7] || null}, ${sector(row[5])}, ${row[6] || null})
      RETURNING id
    `;
  ...
  for (const [name, row] of seen) {
    ...
    const cSlug = nameToSlug.get(name)!;
    const pSlug = nameToSlug.get(parentName)!;
    const cId = slugToId.get(cSlug)!;
    const pId = slugToId.get(pSlug)!;
    if (cId === pId) continue;
    const [a, b2] = [cId, pId].sort();
    const key = `${a}|${b2}`;
    if (seenRels.has(key)) continue;
    seenRels.add(key);
    await db`INSERT INTO related_organizations (org_id_1, org_id_2) VALUES (${a}::uuid, ${b2}::uuid) ON CONFLICT DO NOTHING`;
    relCount++;
  }
