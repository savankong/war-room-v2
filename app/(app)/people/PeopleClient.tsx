...
export default function PeopleClient({ people, topOrgs }: Props) {
  ...
  const [indTagFilter, setIndTagFilter] = useState<string | null>(null);
  ...
  const indPeople = useMemo(() => people.filter(p => isIndustryPerson(p)), [people]);
  ...
  const indTagCounts = useMemo(() => {
    const m: Record<string, number> = {};
    for (const p of indPeople) {
      const tags: string[] = Array.isArray(p.tags) ? p.tags : [];
      for (const t of tags) {
        if (t && t !== 'INDUSTRY') m[t] = (m[t] ?? 0) + 1;
      }
      // Synthetic tags from title
      if (/Principal Investigator/i.test(p.role_title ?? '')) m['Principal Investigator'] = (m['Principal Investigator'] ?? 0) + 1;
      else if (/Business Contact/i.test(p.role_title ?? '')) m['Business Contact'] = (m['Business Contact'] ?? 0) + 1;
      // SBIR tag
      if (p.org_type === 'sbir_company') m['SBIR'] = (m['SBIR'] ?? 0) + 1;
    }
    return m;
  }, [indPeople]);
  ...
  const filtered = useMemo(() => {
    ...
    if (seg === 'gov') {
      ...
      if (indTagFilter) {
        list = list.filter(p => {
          if (indTagFilter === 'SBIR') return p.org_type === 'sbir_company';
          if (indTagFilter === 'Principal Investigator') return /Principal Investigator/i.test(p.role_title ?? '');
          if (indTagFilter === 'Business Contact') return /Business Contact/i.test(p.role_title ?? '');
          return Array.isArray(p.tags) && p.tags.includes(indTagFilter);
        });
      }
      if (titleFilter) {
  ...
  const activeFilterCount = focusFilters.length + (orgFilter ? 1 : 0) + (seniorityFilter ? 1 : 0) + (indTagFilter ? 1 : 0) + (agencyFilter ? 1 : 0) + (titleFilter ? 1 : 0);
          ...
          {seg === 'ind' ? (<>
            ...
            <div className="wr-fg">
              <div className="wr-fg-h">
                <button style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'none', border: 'none', cursor: 'pointer', padding: 0, flex: 1 }} onClick={() => setCompanySectionOpen(v => !v)}>
                  ...
                  <span style={{ marginLeft: 4, color: 'var(--ink-3)', transition: 'transform .15s', transform: companySectionOpen ? 'none' : 'rotate(-90deg)', display: 'flex' }}><IcChevD /></span>
                </button>
                {agencyFilter && <span className="clr" onClick={() => setAgencyFilter(null)}>Clear</span>}
              </div>
              {companySectionOpen && companyOptions.map(([company, cnt]) => {
            ...
            {Object.keys(indTagCounts).length > 0 && (
              <div className="wr-fg">
                <div className="wr-fg-h">
                  <button style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'none', border: 'none', cursor: 'pointer', padding: 0, flex: 1 }} onClick={() => setTagSectionOpen(v => !v)}>
                    <span style={{ color: 'var(--ink-2)', fontSize: 10, fontFamily: 'IBM Plex Mono', letterSpacing: '1.4px', textTransform: 'uppercase', fontWeight: 700 }}>Tags</span>
                    <span style={{ marginLeft: 4, color: 'var(--ink-3)', transition: 'transform .15s', transform: tagSectionOpen ? 'none' : 'rotate(-90deg)', display: 'flex' }}><IcChevD /></span>
                  </button>
                  {indTagFilter && <span className="clr" onClick={() => setIndTagFilter(null)}>Clear</span>}
                </div>
                {tagSectionOpen && Object.entries(indTagCounts).sort((a, b) => b[1] - a[1]).map(([tag, cnt]) => (
                  <div key={tag} className={'wr-chk' + (indTagFilter === tag ? ' on' : '')} onClick={() => setIndTagFilter(indTagFilter === tag ? null : tag)}>
                    <span className="box">{indTagFilter === tag ? <IcTick /> : null}</span>
            ...
            <div className="wr-fg">
              <div className="wr-fg-h">
                <button style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'none', border: 'none', cursor: 'pointer', padding: 0, flex: 1 }} onClick={() => setRoleSectionOpen(v => !v)}>
                  ...
                  <span style={{ marginLeft: 4, color: 'var(--ink-3)', transition: 'transform .15s', transform: roleSectionOpen ? 'none' : 'rotate(-90deg)', display: 'flex' }}><IcChevD /></span>
                </button>
              ...
              {roleSectionOpen && [
                ...
                { label: 'Principal Investigator', match: (t: string) => /Principal Investigator/i.test(t) },
                { label: 'Business Contact', match: (t: string) => /Business Contact/i.test(t) },
              ].map(({ label, match }) => {
